...
exports.sendNotificationIfPollIsExpired = functions.firestore
	...
	.onUpdate(event => {
		...
		if (newValue.isExpired) {
			...
			const tokens = newValue.deviceTokens;
 ...
 */
 /*
exports.sendFollowerNotification = functions.database.ref('/endPoll/{pollUid}').onWrite(event => {
		const pollUid = event.params.pollUid;
		...
		const getDeviceTokensPromise = admin.database().ref(`/polls/${pollUid}/notificationTokens`).once('value');
